...
(:~
 ...
 : 
 : @param $props the xml representation of propertiesAliases.txt
 :
...
(:~
 : determin possible values for @version aka existing official Unicode versions
 : 
 : @param the path to UNICODE CHARACTER DATABASE IN XML relaxNg schema
...
(:~ 
 ...
 :
 : @return a single ucd character element with property attributes
:)
declare function local:make-ucdchar($prop as xs:string) as item()* {
    
    element ucd:ucd {
        element ucd:repertoir {
            element ucd:char {
                attribute cp {'12345'},
                attribute {local:lookup-names($prop)} {'$val-in'}
            }
        }
    }
};
