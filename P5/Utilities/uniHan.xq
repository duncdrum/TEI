...
(:~
 : Look up xml compatible regex pattern for unihan property values
 : and ouput matching schematron constraint
 :
 : @see https://www.unicode.org/reports/tr38/html/body/div/comment()[4-5]
 : @param $name the unihan property name as string
 : 
 : @return one constraintSpec element per unihanProp/@value
 :
:)
declare function unih:uniHan-values($name as xs:string*) as element(constraintSpec)* {
    for $n in $name
    let $rule := $ucna:ucd//@name[. = $n]/..//rng:param[@name="pattern"]/string()
    order by $n
    return
        <constraintSpec ident="{$n}" scheme="schematron">
            <constraint>
                <sch:rule context="tei:unihanProp">
                    <sch:let name="input-value" value="@value"/>
                    <sch:assert test=". != {$n} or matches($input-value, '{$rule}')">
                        <sch:value-of select="$input-value"/> does not match the expected value of the named unihan property.</sch:assert>
                </sch:rule>
            </constraint>
        </constraintSpec>
};
